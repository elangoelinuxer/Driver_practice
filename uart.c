...
#define TINY_SERIAL_MINORS	2	/* only have one minor */
#define UART_NR			1	/* only use one port */

#define TINY_SERIAL_NAME	"ttytiny1"
...
printk("ela: inside tiny_timer fn ");
...
static struct uart_driver tiny_reg1 = {
	.owner		= THIS_MODULE,
...
static int __init tiny_init(void)
{
	...
	result = uart_register_driver(&tiny_reg1);
	if (result)
		return result;

	result = uart_add_one_port(&tiny_reg1, &tiny_port);
	if (result)
		uart_unregister_driver(&tiny_reg1);
